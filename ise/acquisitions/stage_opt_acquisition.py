...
class StageOptAcquisition():
    def __init__(self, gp_model, safe_seed, domain, lipschitz_constant, size_of_discrete_domain, grid_domain=False):
        '''
        Constructor
    ...
    def _compute_kernel_distance(self, x, y):
        ...
        with gpytorch.settings.prior_mode(True):
            ...
            covariance_matrix = self._model(catted_inputs).lazy_covariance_matrix
    ...
    def _point_optimistically_elarges_safe_set(self, safe_set_point, unsafe_point):
        ...
        upper_confidence_bound = self._model.upper_confidence_bound(safe_set_point).squeeze()
        distance = self._compute_kernel_distance(safe_set_point, unsafe_point).squeeze()
